...
class MumbleLink:
    ...
    def __init__(self):
        size1 = ctypes.sizeof(Link)
        size2 = ctypes.sizeof(Context)

        memfile = mmap.mmap(0, size1 + size2, "MumbleLink", mmap.ACCESS_READ)
        memfile.seek(0)

        self.data = self.unpack(Link, memfile.read(size1))
        self.context = self.unpack(Context, memfile.read(size2))
